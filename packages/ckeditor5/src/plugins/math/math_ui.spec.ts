	...
	describe( '_showUI()', () => {
		...
		describe( 'math form view', () => {
			...
			describe( 'binding', () => {
				...
				it( 'should populate mainFormView.mathInputView#value from the math command', () => {
					const command = editor.commands.get( 'math' );

					expect( formView!.mathInputView.value ).to.be.null;

					// Reopening the UI is what pushes the command's equation into the form. The
					// `value` binding set up in _createFormView() cannot be relied on here:
					// MathInputView assigns its own `value` whenever the MathLive field or the
					// LaTeX textarea reports a change, and writing a bound property directly
					// severs the binding — which happens as soon as the field mounts.
					command!.value = 'x^2';
					mathUIFeature._showUI();
